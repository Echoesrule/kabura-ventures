...
def get_analytics():
    ...
    revenue_by_type = db.session.query(
        Payment.payment_type, func.sum(Payment.amount), func.count(Payment.id)
    ...
    ).filter(Payment.status == 'completed', Payment.created_at >= twelve_months_ago
    ).group_by('month').order_by('month').all()

    total_bookings = Booking.query.count()
    bookings_by_status = db.session.query(Booking.status, func.count(Booking.id)).group_by(Booking.status).all()
    bookings_by_type = db.session.query(Booking.booking_type, func.count(Booking.id)).group_by(Booking.booking_type).all()

    monthly_bookings = db.session.query(
        func.to_char(Booking.created_at, 'YYYY-MM').label('month'),
        func.count(Booking.id).label('count')
    ).filter(Booking.created_at >= twelve_months_ago).group_by('month').order_by('month').all()

    popular_tours = db.session.query(
        Booking.tour_id, func.count(Booking.id).label('bookings_count'),
        func.sum(Booking.people_count).label('total_people')
    ).filter(Booking.tour_id.isnot(None), Booking.status != 'cancelled'
    ...
    for tour_id, b_count, t_people in popular_tours:
        ...
        if tour:
            popular_tours_data.append({
                'title': tour.title, 'price': float(tour.price) if tour.price else 0,
                'bookings_count': b_count, 'total_people': t_people,
    ...
    return {
        'revenue': {
            'total': float(total_revenue), 'total_payments': total_payments,
            'by_method': [{'method': m, 'total': float(t), 'count': c} for m, t, c in revenue_by_method],
            'by_type': [{'type': t, 'total': float(a), 'count': c} for t, a, c in revenue_by_type],
            'monthly': [{'month': m, 'total': float(t), 'count': c} for m, t, c in monthly_revenue]
        },
        'bookings': {
            ...
            'by_status': [{'status': s, 'count': c} for s, c in bookings_by_status],
            'by_type': [{'type': t, 'count': c} for t, c in bookings_by_type],
            'monthly': [{'month': m, 'count': c} for m, c in monthly_bookings]
        },
        'platform': {
            'total_users': total_users, 'total_tours': total_tours,
            'total_hotels': total_hotels, 'total_flight_requests': total_flight_requests,
            'pending_flights': pending_flights, 'unread_messages': new_messages,
        },
